function NCS() {
	...
	state.updateVersionDropDown = function () {
		...
		const by_version =
			ncs.versions.COMPONENTS_BY_VERSION[ncs.current_version] ||
		...
		$.each(versions, function (_, v) {
			if (v !== ncs.current_version) {
				let link = `<a class="dropdown-item" href=${ncs.url_root + v}/${
					ncs.current_page
				}>${v}</a>`
	...
	state.showVersion = function () {
		...
		const last_release_url =
			ncs.url_root + VERSIONS[last_version_index] + path_suffix
		const latest_release_url = ncs.url_root + 'latest' + path_suffix

		const SWITCH_MSG =
			'You might want to switch to the documentation for ' +
			"the <a href='" +
			last_release_url +
			"'>" +
			VERSIONS[last_version_index] +
			"</a> release or the <a href='" +
			latest_release_url +
			"'>current " +
		...
		const DEV_RELEASE_MSG =
			'You are looking at the documentation for a development tag. ' +
			SWITCH_MSG
		...
		if (ncs.current_version === VERSIONS[last_version_index]) {
			...
			VERSIONS.includes(ncs.current_version) &&
